...
%  Source code of the LEOMPA (Matlab  R2020b)				     	%
%                                                                   %
...
function [Top_predator_fit,Top_predator_pos,Convergence_curve]=LEOMPA(SearchAgents_no,Max_iter,lb,ub,dim,fobj,newNopar)
if(~exist('newNopar','var'))
    newNopar = 0.1;  % 如果未出现该变量，则对其进行赋值
end
...
while Iter<Max_iter    
 ...
 disp(['Iteration ' num2str(Iter) ': Best Cost = ' num2str(Top_predator_fit)]);%展示现阶段最优误差函数
 ...
 RB=randn(SearchAgents_no,dim);          %Brownian random number vector
  ...
  Convergence_curve(Iter)=Top_predator_fit; 
	 ...
	 %---------- LEO ------------    
		...
		% LEO creates solutions
		[newSolutions, orderY]=LEO(solutionsX,solutionsY,ceil(length(solutionsY)*newNopar),Iter,Max_iter,lb,ub); 
		...
		for noN=1: size(newSolutions,1) 
			...
			Prey(whichOne,:)=whichOneX;  	 % LEO replaces worst solutions	 			
			
			Flag4ub=Prey(whichOne,:)>ub;     % boundary checking
			Flag4lb=Prey(whichOne,:)<lb;  
			Prey(whichOne,:)=(Prey(whichOne,:).*(~(Flag4ub+Flag4lb)))+ub.*Flag4ub+lb.*Flag4lb;
